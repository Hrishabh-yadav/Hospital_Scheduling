...
def employee():
    ...
    EMP = Employee.objects.create(empid=empid, empname=emname,empmail=emmail, etype=etype, edob=edob, gender=gender, fine=fine, finedate=finedate)
    EMP.save()

def depenedent():
    query1 = Dependent.objects.all()
    query2 = Employee.objects.filter(empid=1001)
